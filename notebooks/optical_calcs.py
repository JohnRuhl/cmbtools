...
def optical_calcs(detector, optics):
    ...
    noise_prefactor = 2*((h**2)*(detector['nu']**2))*(2*detector['Nmodes']*detector['Npol'])
    ...
    for i in range(len(optics)):
        ...
        integrand = noise_prefactor*(n + detector['BoseFactor']*n**2) # if you want that term... )
        NEP2_photon = np.trapz(integrand, detector['nu'])
        optics[i]['NEP_photon'] = np.sqrt(NEP2_photon)
    ...
    integrand2 = AOmega*detector['bandshape']*(prefactor*h**2*(np.power(detector['nu'],4)))*np.exp(x)/\
      (k*(c**2)*(T_cmb**2)*(np.exp(x)-1.0)**2)
    detector['dPdT_cmb'] = np.trapz(integrand2, detector['nu'])

    # Record all the converted total quantities
    detector['eta_tot'] = eta_tot
    detector['NEP_photon_total'] = np.sqrt(NEP2_photon_total)  # historical reasons
